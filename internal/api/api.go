...
func generateCollage(ctx context.Context, request *CollageRequest, logger *zerolog.Logger) (image.Image, error) {
	...
	period := session.GetPeriodFromStr(request.Period)
	method := session.GetCollageTypeFromStr(request.Method)
	switch method {
	case session.ALBUM:
...
func Collage(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		switch {
		case err == session.ErrUserNotFound:
			http.Error(w, "User not found", http.StatusNotFound)
		case err == session.ErrTooManyImages:
